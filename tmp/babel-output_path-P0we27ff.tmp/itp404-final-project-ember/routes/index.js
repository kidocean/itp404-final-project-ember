  ...
  exports['default'] = _ember['default'].Route.extend({
    model: function model() {
      function renderMap() {
        ...
        var promise = $.ajax({
          url: 'http://api.reliefweb.int/v1/reports?profile=full&limit=30&filter[field]=date.created&filter[value][from]=2010-06-01T00:00:00%2B00:00&filter[value][to]=2016-06-30T23:59:59%2B00:00',
          dataType: 'json',
          type: 'get'
        }).then(function (data) {
          return data.data;
          ...
          var mapped = datadata.map(function (item) {
            ...
            if (origin !== undefined) {
              ...
              marker.bindPopup("<strong>" + title + "</strong>" + "<br><a href=" + origin + " target='_new'>Read More</a><br>" + "Donate to <strong>" + "<a href =" + orglink + " target='_new'>" + orgname + "</a> " + "</strong>").openPopup();
            } else if (url !== undefined) {
              var marker = L.marker([lat, lon]).addTo(mymap);
              marker.bindPopup("<strong>" + title + "</strong>" + "<br><a href=" + url + " target='_new'>Read More</a><br>" + "Donate to <strong>" + "<a href =" + orglink + " target='_new'>" + orgname + "</a> " + "</strong>").openPopup();
            } else {
              var marker = L.marker([lat, lon]).addTo(mymap);
